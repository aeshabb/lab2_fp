...
let test_insert_find _ =
  let d =
    empty |> insert "apple" 1 |> insert "app" 2 |> insert "banana" 3
    |> insert "band" 4
  in
...
let test_remove _ =
  let d =
    empty |> insert "apple" 1 |> insert "app" 2 |> insert "application" 3
  in
...
let test_filter _ =
  let d =
    empty |> insert "apple" 1 |> insert "apricot" 2 |> insert "banana" 3
  in
...
let test_equal_many_elements _ =
  let pairs =
    [ ("apple", 1); ("app", 2); ("banana", 3); ("band", 4); ("cat", 5) ]
  in
  let d1 = of_list pairs in
  let d2 = of_list (List.rev pairs) in
  let d3 =
    of_list [ ("band", 4); ("app", 2); ("cat", 5); ("banana", 3); ("apple", 1) ]
  in
...
let test_find_prefix _ =
  let d =
    empty |> insert "apple" 1 |> insert "application" 2 |> insert "apply" 3
    |> insert "banana" 4
  in
...
(* Генератор словаря *)
let pairs_gen =
  Q.Gen.list_size (Q.Gen.int_range 0 20) (Q.Gen.pair key_gen Q.Gen.int)
...
let prop_monoid_left_identity =
  Q.Test.make ~name:"Моноид: merge empty d = d" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.merge PTD.empty d) d)

(* Моноид: правый нейтральный элемент *)
let prop_monoid_right_identity =
  Q.Test.make ~name:"Моноид: merge d empty = d" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.merge d PTD.empty) d)

(* Моноид: ассоциативность *)
let prop_monoid_associativity =
  Q.Test.make ~name:"Моноид: ассоциативность merge" ~count:100
    (Q.triple dict_arb dict_arb dict_arb) (fun (a, b, c) ->
      PTD.equal Int.equal
        (PTD.merge (PTD.merge a b) c)
        (PTD.merge a (PTD.merge b c)))

(* Вставка затем поиск возвращает значение *)
let prop_insert_find =
  Q.Test.make ~name:"insert k v; find k = Some v" ~count:200
    (Q.pair key_arb Q.int) (fun (k, v) ->
...
let prop_remove_find =
  Q.Test.make ~name:"remove k; find k = None" ~count:200
    (Q.pair dict_arb key_arb) (fun (d, k) ->
      let d' = PTD.remove k d in
      PTD.find_opt k d' = None)

(* size = длина to_list *)
let prop_size_to_list =
  Q.Test.make ~name:"size = length (to_list)" ~count:100 dict_arb (fun d ->
      PTD.size d = List.length (PTD.to_list d))

(* of_list (to_list d) = d *)
let prop_of_list_to_list =
  Q.Test.make ~name:"of_list (to_list d) = d" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.of_list (PTD.to_list d)) d)

(* map id = id *)
let prop_map_identity =
  Q.Test.make ~name:"map id = id" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.map (fun x -> x) d) d)

(* map (f . g) = map f . map g *)
let prop_map_composition =
  Q.Test.make ~name:"map (f . g) = map f . map g" ~count:100 dict_arb (fun d ->
      let f x = x + 1 in
      let g x = x * 2 in
      PTD.equal Int.equal
        (PTD.map f (PTD.map g d))
        (PTD.map (fun x -> f (g x)) d))

(* filter true = id *)
let prop_filter_true =
  Q.Test.make ~name:"filter (const true) = id" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.filter (fun _ _ -> true) d) d)

(* filter false = empty *)
let prop_filter_false =
  Q.Test.make ~name:"filter (const false) = empty" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal (PTD.filter (fun _ _ -> false) d) PTD.empty)
...
let prop_equal_reflexive =
  Q.Test.make ~name:"equal рефлексивно: d = d" ~count:100 dict_arb (fun d ->
      PTD.equal Int.equal d d)

(* Симметричность равенства *)
let prop_equal_symmetric =
  Q.Test.make ~name:"equal симметрично" ~count:100 (Q.pair dict_arb dict_arb)
...
let prop_mem_find =
  Q.Test.make ~name:"mem k = is_some (find_opt k)" ~count:200
    (Q.pair dict_arb key_arb) (fun (d, k) ->
      PTD.mem k d = Option.is_some (PTD.find_opt k d))
...
let () =
  run_test_tt_main ("Prefix Tree Dict тесты" >::: [ unit_tests; property_tests ])
