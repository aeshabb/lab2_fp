(* Prefix Tree Dict - полиморфный словарь на основе префиксного дерева (trie).
   Функтор для полиморфизма по ключам и значениям.
...
module Make (Key : KEY) = struct
  ...
  type 'a t = { value : 'a option; children : 'a t ElemMap.t }

  (* Пустой словарь - нейтральный элемент моноида *)
  let empty = { value = None; children = ElemMap.empty }
  let is_empty t = Option.is_none t.value && ElemMap.is_empty t.children

  (* Создание словаря с одним элементом *)
  let singleton key value =
    let rec aux chars =
      match chars with
      | [] -> { value = Some value; children = ElemMap.empty }
      | c :: rest -> { value = None; children = ElemMap.singleton c (aux rest) }
    in
    aux (Key.to_list key)
  ...
  let insert key value t =
    let rec aux chars node =
      ...
      | c :: rest ->
          let child =
            match ElemMap.find_opt c node.children with
            | Some child -> aux rest child
            | None -> aux rest empty
          in
          { node with children = ElemMap.add c child node.children }
    in
    aux (Key.to_list key) t

  (* Поиск значения по ключу с исключением Not_found *)
  let find key t =
    let rec aux chars node =
      match chars with
      | [] -> ( match node.value with Some v -> v | None -> raise Not_found)
      | c :: rest -> (
          match ElemMap.find_opt c node.children with
          | Some child -> aux rest child
  ...
  (* Поиск значения по ключу через find *)
  ...
  let remove key t =
    let rec aux chars node =
      ...
      | [] ->
          ...
          if ElemMap.is_empty new_node.children then None else Some new_node
      | c :: rest -> (
          match ElemMap.find_opt c node.children with
          | None -> Some node
          | Some child -> (
              ...
              | None ->
                  let new_children = ElemMap.remove c node.children in
                  if Option.is_none node.value && ElemMap.is_empty new_children
                  then None
              ...
              | Some new_child ->
                  Some
                    {
                      node with
                      children = ElemMap.add c new_child node.children;
                    }))
    in
    match aux (Key.to_list key) t with None -> empty | Some t' -> t'
  ...
  let to_list t =
    let rec aux prefix node acc =
      let acc =
        ...
        | Some v -> (Key.of_list prefix, v) :: acc
        | None -> acc
      in
      ElemMap.fold
        (fun c child acc -> aux (prefix @ [ c ]) child acc)
        node.children acc
    in
    List.rev (aux [] t [])
  ...
  let size t =
    let rec aux node =
      ...
      ElemMap.fold (fun _ child acc -> acc + aux child) node.children count
    in
    aux t
    ...
    let rec aux node =
      {
        ...
        children = ElemMap.map aux node.children;
      }
    in
    aux t
  ...
  let mapi f t =
    let rec aux prefix node =
      {
        value = Option.map (f (Key.of_list prefix)) node.value;
        children =
          ElemMap.mapi (fun c child -> aux (prefix @ [ c ]) child) node.children;
      }
    in
    aux [] t
  ...
  let filter pred t =
    let rec aux prefix node =
      let new_value =
        match node.value with
        | Some v when pred (Key.of_list prefix) v -> Some v
        | _ -> None
      in
      let new_children =
        ElemMap.fold
          (fun c child acc ->
            let new_prefix = prefix @ [ c ] in
            let new_child = aux new_prefix child in
            if is_empty new_child then acc else ElemMap.add c new_child acc)
          node.children ElemMap.empty
      in
  ...
  let filter_values pred t = filter (fun _ v -> pred v) t
  ...
  let fold_left f init t =
    let rec aux prefix acc node =
      let acc =
        match node.value with
        | Some v -> f acc (Key.of_list prefix) v
        | None -> acc
      in
      ElemMap.fold
        (fun c child acc -> aux (prefix @ [ c ]) acc child)
        node.children acc
    in
    aux [] init t
  ...
  let fold_right f t init =
    let rec aux prefix node acc =
      let acc =
        ElemMap.to_rev_seq node.children
        |> Seq.fold_left
             (fun acc (c, child) -> aux (prefix @ [ c ]) child acc)
             acc
      in
      match node.value with
      | Some v -> f (Key.of_list prefix) v acc
      | None -> acc
    in
    aux [] t init

  (* Итерация по всем парам *)
  let iter f t = fold_left (fun () k v -> f k v) () t
  ...
  let merge t1 t2 = merge_with (fun _v1 v2 -> v2) t1 t2

  (* Конкатенация списка словарей *)
  let concat dicts = List.fold_left merge empty dicts
  ...
  let equal value_eq t1 t2 =
    let rec aux n1 n2 =
      ...
      if not values_eq then false
        ...
        let keys1 = ElemMap.bindings n1.children |> List.map fst in
        let keys2 = ElemMap.bindings n2.children |> List.map fst in
        if keys1 <> keys2 then false
        else
          ElemMap.for_all
            (fun c child1 ->
              match ElemMap.find_opt c n2.children with
              | None -> false
  ...
  let keys t = fold_left (fun acc k _ -> k :: acc) [] t |> List.rev

  (* Получение всех значений *)
  let values t = fold_left (fun acc _ v -> v :: acc) [] t |> List.rev
  ...
  let find_prefix prefix t =
    let rec navigate chars node =
      ...
      | c :: rest -> (
          ...
          | None -> None)
    in
    match navigate (Key.to_list prefix) t with
    | None -> []
    | Some subtree ->
        let rec collect current_prefix node acc =
          let acc =
            match node.value with
            | Some v -> (Key.of_list current_prefix, v) :: acc
            | None -> acc
          in
          ElemMap.fold
            (fun c child acc -> collect (current_prefix @ [ c ]) child acc)
            node.children acc
        in
        let prefix_list = Key.to_list prefix in
        List.rev (collect prefix_list subtree [])

  (* Проверка наличия ключей с данным префиксом *)
  let has_prefix prefix t = find_prefix prefix t <> []
end

(* Модуль для строковых ключей *)
module StringKey : KEY with type t = string and type elem = char = struct
  type t = string
  type elem = char

  let to_list s = List.of_seq (String.to_seq s)
  let of_list chars = String.of_seq (List.to_seq chars)
  let compare_elem = Char.compare
end
