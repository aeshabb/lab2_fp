...
type 'a t = { value : 'a option; children : 'a t CharMap.t }

(* Пустой словарь - нейтральный элемент моноида *)
let empty = { value = None; children = CharMap.empty }
let is_empty t = Option.is_none t.value && CharMap.is_empty t.children
...
let singleton key value =
  let rec aux chars =
    ...
    | c :: rest -> { value = None; children = CharMap.singleton c (aux rest) }
  in
    ...
    | c :: rest -> (
        ...
        | Some child -> (
            ...
            | None ->
                ...
                if Option.is_none node.value && CharMap.is_empty new_children
                then None
...
let to_list t =
  let rec aux prefix node acc =
    let acc =
      match node.value with Some v -> (prefix, v) :: acc | None -> acc
    in
    CharMap.fold
      (fun c child acc -> aux (prefix ^ String.make 1 c) child acc)
      node.children acc
  in
  List.rev (aux "" t [])
  ...
  let rec aux prefix node =
    {
      ...
      children =
        CharMap.mapi
          (fun c child -> aux (prefix ^ String.make 1 c) child)
          node.children;
    }
  in
  aux "" t
...
let filter pred t =
  let rec aux prefix node =
    let new_value =
      match node.value with Some v when pred prefix v -> Some v | _ -> None
    in
    let new_children =
      CharMap.fold
        (fun c child acc ->
          ...
          if is_empty new_child then acc else CharMap.add c new_child acc)
...
let fold_left f init t =
  let rec aux prefix acc node =
    ...
    CharMap.fold
      (fun c child acc -> aux (prefix ^ String.make 1 c) acc child)
      node.children acc
  in
  aux "" init t
...
let fold_right f t init =
  let rec aux prefix node acc =
    let acc =
      ...
      |> Seq.fold_left
           (fun acc (c, child) -> aux (prefix ^ String.make 1 c) child acc)
           acc
    in
...
let merge t1 t2 =
  let rec aux n1 n2 =
    let new_value =
      match n2.value with Some _ -> n2.value | None -> n1.value
    in
    let new_children =
      CharMap.merge
        (fun _ c1 c2 ->
